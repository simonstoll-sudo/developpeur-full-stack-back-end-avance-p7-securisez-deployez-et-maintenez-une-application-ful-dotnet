...
- Séparez la publication de l'application (`dotnet publish`) de la construction de l'image : un build multi-étapes dans le `Dockerfile` évite d'embarquer le SDK .NET complet dans l'image finale.
- Externalisez la configuration (port, identifiants) en variables d'environnement plutôt qu'en dur dans `appsettings.json`.
...
- un fichier de workflow dans `.github/workflows/` qui, à chaque push, lance `dotnet test` puis construit l'image Docker ;
- une exécution verte du workflow, visible dans l'onglet Actions du dépôt.

**Recommandations :**

- Commencez par un workflow minimal (checkout, installation du SDK .NET, `dotnet test`), vérifiez qu'il passe, puis ajoutez la construction de l'image. Un workflow se construit par itérations courtes.
...
- l'exposition de métriques de base de l'application (santé, temps de réponse, utilisation mémoire) via l'instrumentation OpenTelemetry ou les health checks natifs d'ASP.NET Core ;
...
# Mission 1 — Garantissez la maintenance, la sécurité et la qualité d'une application (.NET)
...
- Outillez votre audit : `dotnet test` pour l'état de la suite existante, le rapport Coverlet pour la couverture réelle, une analyse SonarQube pour objectiver la dette. Un chiffre constaté vaut mieux qu'une impression.
...
- Parcourez l'architecture en couches (`Controllers`, `Services`, `Data`, `Models`) : les défauts de couche (entité EF Core exposée au lieu d'un DTO, code d'erreur renvoyé depuis un service) sont des marqueurs de dette à noter dès maintenant.
...
- une authentification OAuth2 avec jetons JWT mise en œuvre via ASP.NET Core Identity, et des autorisations différenciées par rôle ;
- la validation des entrées par attributs de modèle (`[Required]`, `[StringLength]`, vérification de `ModelState`) plutôt que par des vérifications manuelles dispersées ;
- la protection des données stockées : mots de passe hachés avec un algorithme adapté (le hachage natif d'ASP.NET Core Identity), secrets sortis du code et de `appsettings.json`, et secrets déjà exposés dans l'historique Git traités (révocation et remplacement, purge de l'historique si vous la jugez nécessaire) ;
...
- Pour chaque attribut de sécurité que vous posez, sachez dire ce qu'il déclenche : une configuration ASP.NET Core Identity copiée sans être comprise se voit immédiatement en soutenance.
...
- des tests unitaires xUnit avec Moq pour les doublures, ciblant en priorité les règles métier des services ;
- des tests d'intégration et des tests d'API sur les contrôleurs via `WebApplicationFactory`, sur une base de données réelle éphémère (Testcontainers, ou une alternative que vous justifiez) ;
- une suite qui passe intégralement avec `dotnet test` ;
- un **rapport de couverture de code** généré automatiquement par Coverlet lors du build.

**Recommandations :**

- Distinguez un double de test isolé et un test qui démarre l'application complète via `WebApplicationFactory` : le premier ne charge pas le pipeline ASP.NET Core, le second démarre le contexte entier. Un test qui charge toute l'application pour vérifier une règle de calcul est un test lent pour rien.
- L'injection par constructeur, plutôt que par résolution directe du conteneur, est ce qui rend vos services testables sans dépendance au framework : si un service résiste au test unitaire, c'est souvent sa construction qu'il faut corriger d'abord.
- Visez la couverture des zones à risque avant le pourcentage global : 100 % sur les propriétés n'a jamais protégé personne.

**Points de vigilance :**

- Une suite de tests qui ne passe pas avec `dotnet test` au moment de la soutenance peut motiver un refus : le filet de sécurité qui ne tient pas n'est pas un filet.
...
- un pipeline qui, sur chaque push et pull request : compile le projet (`dotnet build`), exécute `dotnet test`, génère le rapport Coverlet et construit l'image Docker de l'application, dans cet ordre, et échoue si les tests échouent ;
...
- Le `Dockerfile` suit la même logique qu'à l'exercice optionnel : le résultat de `dotnet publish`, empaqueté sur une image d'exécution .NET, la configuration en variables d'environnement.
...
- les défauts identifiés à l'audit corrigés dans le code : gestion des erreurs centralisée (exceptions métier en classes, traduites en réponses HTTP par un middleware de gestion d'exceptions), validation des entrées généralisée, logs rendus exploitables (niveaux cohérents, contexte suffisant, aucune donnée sensible journalisée) ;
...
- Refactorisez sous protection : les tests d'abord, la refactorisation ensuite, `dotnet test` au vert après chaque mouvement. Une refactorisation sans tests est un pari, pas de la maintenance.
- Le classique du parcours : l'exception de suivi de chargement différé, cette relation EF Core chargée paresseusement qui explose hors contexte. Si vous la rencontrez, expliquez-la dans le journal ; ne la contournez pas au hasard.
